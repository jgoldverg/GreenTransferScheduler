...
@click.option('--trace_route', type=click.Path(), default='../config/trace_route_with_coordinates.json',
              show_default=True,
...
@click.option('--forecast-file', type=click.Path(), default='../config/forecasts_config.json',
              show_default=True,
              help="Path to the file containing electricity maps forecast for every ip in a trace file.")
@click.option('--save-forecasts', type=click.BOOL, default=False,
              help="Saves the forecasts into the forecast-file path provided above")
@click.argument("simgrid_options", nargs=-1, type=click.UNPROCESSED)
def schedule(trace_route, job_file, node_file, forecast_file, save_forecasts, simgrid_options):
    """Schedule a job using the given file paths."""
    simgrid_args = [sys.argv[0]] + list(simgrid_options)
    ...
    scheduler_algo.create_plan(simgrid_options)
